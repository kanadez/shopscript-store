...
SQL;
    ...
    public function productImgWrapperClass($product){
        $product_images_model = new shopProductImagesModel();
        $image_id = $product["image_id"];
        $product_image = $product_images_model->getById($image_id);
        $response = $product_image["width"] > $product_image["height"] ? "double" : "single";
    ...
    public function getFirstCategoryProductImage($category){
        $category_id = $category["id"];
        $shop_product_model = new shopProductModel();
        $shop_category_products_model = new shopCategoryProductsModel();
        
        $category_products = $shop_category_products_model->getByField("category_id", $category_id);
        $product_id = $category_products["product_id"];
        $product = new shopProduct($product_id);
        
        return $this->productImgUrl($product, "400");
    }
    ...
    public function getNextProduct($product){
        ...
        if (!isset($from)){
            $response = $this->getRegularNextProduct($product);
        }
        else{
            switch ($from){
                case "discounts":
                    $response = $this->getDiscountsNextProduct($product);
                break;
                default:
                    $response = $this->getRegularNextProduct($product);
                break;
    ...
    public function getRegularNextProduct($product){
        $product_id = $product["id"];
        $product_category = $product["category_id"];
        $response = FALSE;
        $category_products_model = new shopCategoryProductsModel();
        
        $category_products = $category_products_model->getByField("category_id", $product_category, true);
        
        if (count($category_products) > 1){
            foreach ($category_products as $k => $p){
                if ($p["product_id"] == $product_id){
                    if ($k == count($category_products)-1){
                        $response = $this->getRegularNextProductUrl(0, $category_products);
                    }
                    else{
                        $response = $this->getRegularNextProductUrl($k+1, $category_products);
                    }
                }
            }
        }
    ...
    private function getRegularNextProductUrl($product_key, $category_products){
        $next_product_id = $category_products[$product_key]["product_id"];
        $next_product = new shopProduct($next_product_id);
        
        return $next_product->getProductUrl();
    }
    
    public function getDiscountsNextProduct($product){
        $category_products = null;
        $product_id = $product["id"];
        $response = FALSE;
        $discounts = $this->productSet("discounts");
        $discounts_parsed = array();

        foreach ($discounts as $d){
            array_push($discounts_parsed, $d);
        }

        $category_products = $discounts_parsed;
        
        if (count($category_products) > 1){
            foreach ($category_products as $k => $p){
                if ($p["id"] == $product_id){
                    if ($k == count($category_products)-1){
                        $response = $this->getDiscountsNextProductUrl(0, $category_products);
                    }
                    else{
                        $response = $this->getDiscountsNextProductUrl($k+1, $category_products);
                    }
                }
            }
        }
    ...
    private function getDiscountsNextProductUrl($product_key, $category_products){
        $next_product_id = $category_products[$product_key]["id"];
        $next_product = new shopProduct($next_product_id);
        $next_product_url = $next_product->getProductUrl();

        return $next_product_url."?from=discounts";
    }
    
    public function allFeatures($id){
        $feature_values_model = new shopFeatureValuesVarcharModel();
        
        return $feature_values_model->getByField("feature_id", $id, true);
    }
    
    public function getFavoritesCategories($products){
        $category_ids = array();
        $categories = array();
        $category_model = new shopCategoryModel();
        ...
        foreach ($products as $p){
            ...
            if (array_search($category_id, $category_ids) === FALSE){
                $category = $category_model->getById($category_id);
                $response = array("id" => $category["id"], "name" => $category["name"]);
                array_push($category_ids, $category_id);
                array_push($categories, $response);
            }
        }
    ...
    public function isOrderDigital($order_id){
        $shop_order_items_model = new shopOrderItemsModel();
        $items = $shop_order_items_model->getByField("order_id", $order_id);
        
        $product_id = $items["product_id"];        
        $product = new shopProduct($product_id);
        $product_type = $product->getData("type_id");
        $is_digital = $product_type == self::DIGITAL_PRODUCT_TYPE ? TRUE : FALSE;
