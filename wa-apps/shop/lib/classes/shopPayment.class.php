    ...
    public function getBackUrl($type = self::URL_SUCCESS, $transaction_data = array())
    {
        ...
        switch ($type) {
            ...
            case self::URL_SUCCESS:
                $digital_request = "/shop/checkout_digital/success/";
                $request_uri = waRequest::server("REQUEST_URI");
                $route_url = $request_uri == $digital_request ? 'shop/frontend/checkout_digital' : 'shop/frontend/checkout';
                
                $url = wa()->getRouteUrl($route_url, array('step' => 'success'), true);
