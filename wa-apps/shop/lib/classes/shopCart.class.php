...
class shopCart
{
    ...
    public function getItemDiscountTotal($item_id){
        if (is_array($item_id)){
            $item_id = $item_id['id'];
            $item = $this->getItem($item_id);
        } 
        ...
        $price = shop_currency($item['compare_price'], $item['currency'], null, false) * $item['quantity'];
    ...
    public function checkTypeMixing($product){
        $product_type_id = $product["type_id"];
        $items = $this->items();
        $response = FALSE;
        
        foreach ($items as $item){
            if ($item["product"]["type_id"] != $product_type_id){
                $response = TRUE;
            }
        }
