...
class shopProduct implements ArrayAccess
{
    ...
    public function getFiles($absolute = false){
        if ($this->getId()) {
            $files_model = new shopProductFilesModel();
            $files = $files_model->getFiles($this->getId(), 'id', $absolute);
            
            foreach ($files as &$file){
                if ($file['description'] == '') {
                    $file['description'] = $this->name;
                }
