...
class shopFile
{
    ...
    /**
     * Returns path to product file
     *
     * @param array $file Key-value image data object
     * @return string
     */
    public static function getPath($file){        
        return shopProduct::getPath($file['product_id'], "files/{$file['filename']}.{$file['ext']}", FALSE);
    }
     ...
     * @return string
     */
    public static function getUrl($image, $absolute = false){
        ...
        $path = shopProduct::getFolder($image['product_id'])."/{$image['product_id']}/files/{$n}.{$image['ext']}";

        if (waSystemConfig::systemOption('mod_rewrite')){
            return wa()->getDataUrl($path, FALSE, 'shop', $absolute);
        }
        else{
