...
class shopFrontendCartAddController extends waJsonController
{
    ...
    public function execute()
    {
        ...
        if (!empty($product) && !empty($sku)) {
            ...
            // check for product type mixing
            if ($this->cart->checkTypeMixing($product)){
                if ($product["type_id"] == 1){
                    $this->errors = _w("You have some physical products in your cart. You can't mix them with digital products. Checkout or clear cart.");
                }
                else{
                    $this->errors = _w("You have some digital products in your cart. You can't mix them with physical products. Checkout or clear cart.");
                }
