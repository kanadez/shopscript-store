...
/**
 * Class shopFrontendCheckoutDigitalAction
 * @method shopConfig getConfig()
 */
class shopFrontendCheckoutDigitalAction extends waViewAction
{
    const PAID_ORDER_STATE_ID = "paid";
    ...
    public function execute()
    {
                ...
                if (waRequest::post('wa_auth_login')) {
                    ...
                    foreach ($steps as $step_id => $step) {
                        if ($step_id == $current_step) {
                            ...
                            $this->redirect(wa()->getRouteUrl('/frontend/checkout_digital', array('step' => $step_id)));
                        } elseif ($last_step) {
                            $step_instance = $this->getStep($step_id);
                    ...
                    if ($redirect && !$errors) {
                        if ($order_id = $this->createOrder($errors)) {
                            ...
                            $this->redirect(wa()->getRouteUrl('/frontend/checkout_digital', array('step' => shopCheckout::STEP_SUCCESS)));
                        } else {
        ...
        if (waRequest::isXMLHttpRequest()) {
            $this->setThemeTemplate('checkout_digital.'.$current_step.'.html');
        } else {
            $this->setLayout(new shopFrontendLayout());
            $this->setThemeTemplate('checkout_digital.html');
        }
    }
    ...
    protected function success(){
        ...
        if (!$payment_success){ // отдаем интерфейс плагина для оплаты
            ...
            if ($this->isPaymentCompleted($order_id)){
                $order['payment_completed'] = TRUE; // в шаблоне проверяем этот флаг и скрываем блок ссылки если FALSE
                $order['payment_error_message'] = _w("Что-то не так с файлами товаров. Напишите нам, мы разберемся.");
                $order['purchased_product_links'] = $this->getPurchasedProductLinks($order_id);
            }
            else{
                $order['payment_completed'] = FALSE; // в шаблоне проверяем этот флаг и отдаем сообщение об ошибке оплаты
                $order['payment_error_message'] = _w("Товар еще не оплачен, либо что-то пошло не так с оплатой.<br> Напишите нам, мы разберемся.");
            }
        }
    ...
    protected function isPaymentCompleted($order_id){
        // здесь должен проверяться статус платежа, возвращенный платежной системой.
        // пока что в качестве заглушки будем возрващать TRUE
        // причем необходимо проверить, является ли текущий юзер хозяином заказа
        $cart = new shopCart();
        $order = new shopOrder($order_id);
        $order_contact = $order->getData("contact_id");
        $order_contact_cart_code = $order->getData("contact_cart_code");
        $current_user = null;
        $response = FALSE;
        ...
        (
            isset($current_user) &&
            (
                $order_contact == $current_user["id"] || 
                (isset($order_contact_cart_code) && $order_contact_cart_code == $current_user)
            )
        )
        {
            // здесь проверяем прошел ли платеж. если нет возвращаем FALSE
            // а сам код ошибки платежа дожен записываться в error.log или payments.log в функции внутри ipn-лисенера
            $order_state_id = $order->getData("state_id");
            
            if ($order_state_id == self::PAID_ORDER_STATE_ID){
                $response = TRUE;
            }
        }
    ...
    protected function getPurchasedProductLinks($order_id){
        // здесь берем из базы ссылку на оригинал цифрового товара и отдаем
        $shop_order_products = new shopOrderItemsModel();
        $shop_product_files_model = new shopProductFilesModel();
        $items = $shop_order_products->getItems($order_id);
        $original_links = [];

        foreach ($items as $item){
            $product_files = $shop_product_files_model->getByField("product_id", $item["product_id"], true);
            $tmp_directory_name = uniqid(); // создаем случайное имя директории
            // составляем маршрут $target_path = путь к папке данных contacts + "/files/" + случайное имя директории
            $contacts_data_path = wa()->getDataPath(null, true, "contacts", false);
            $target_path = $contacts_data_path."/files/".$tmp_directory_name;
            
            foreach ($product_files as $file){
                $file_path = shopFile::getPath($file);
                $full_target_path = $target_path."/".$file["filename"].".".$file["ext"];
                
                waFiles::copy($file_path, $full_target_path);
            }
            
            $contacts_url = wa()->getDataUrl(null, true, "contacts", false);
            $zip_file_name = $this->addProductFilesToZip($target_path, $product_files);
            
            if (!$zip_file_name){
                return FALSE;
            }
            
            $zip_file_url = $contacts_url."/files/".$tmp_directory_name."/".$zip_file_name;
            $zip_file_data = array("file_url" => $zip_file_url, "file_name" => $zip_file_name);
            array_push($original_links, $zip_file_data);
        }

        return $original_links;
    }
    
    protected function addProductFilesToZip($working_path, $product_files){
        $zip = new ZipArchive;
        $product_name = $product_files[0]["filename"];
        $zip_name = $working_path."/".$product_name.'.zip';
        ...
        if ($zip_file === TRUE){
            ...
            $zip->close();
            $response = $product_name.'.zip';
        }
        else{
            $error_message = "Error creating zip-file, filename: ".$zip_name.", error code: ".$zip_file;
            waLog::log($error_message);
        }
