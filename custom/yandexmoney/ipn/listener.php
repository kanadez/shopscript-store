...
$yam_secret = "G8jcpJdjA0fJAMv1W0cn0em6";
$yam_db_table = "custom_yandexmoney_ipn";
$myPost = array();


klog("", "\n########################################");
klog("time", date('m/d/Y h:i:s a', time()));
...
foreach ($raw_post_array as $keyval) {
    ...
    if (count($keyval) == 2){
        ...
        klog($keyval[0], $myPost[$keyval[0]]);
    }
}

$parameters_string = $myPost["notification_type"]."&"
. $myPost["operation_id"]."&"
. $myPost["amount"]."&"
. $myPost["currency"]."&"
. $myPost["datetime"]."&"
...
. $yam_secret."&"
. $myPost["label"];
klog("sha1", sha1($parameters_string));


if (sha1($parameters_string) === $myPost["sha1_hash"]){ // если платеж успешно прошел
    $sql = sprintf("INSERT INTO `$yam_db_table` ("
            . "`operation_id`, "
            . "`notification_type`, "
            . "`user`, "
            . "`amount`, "
            ...
            . "`timestamp`"
            . ") "
            . "VALUES ('%s', '%s', %d, %f, %f, '%s', %d);", 
        mysql_real_escape_string($myPost["operation_id"]),
        mysql_real_escape_string($myPost["notification_type"]),
        mysql_real_escape_string(0),
        mysql_real_escape_string($myPost["amount"]),
        mysql_real_escape_string($myPost["withdraw_amount"]),
        mysql_real_escape_string($myPost["label"]),
        mysql_real_escape_string(time()));
    $sql_response = $db->db_query($sql, __LINE__, __FILE__);
    klog("sql query response", $sql_response);
    
    $label_exploded = explode("_", $myPost["label"]);
    klog("label part 1", $label_exploded[0]);
    klog("label part 2", $label_exploded[1]);
    klog("label part 3", $label_exploded[2]);
    $order_id = $label_exploded[2];
...
function setOrderPaid($order_id){
    ...
    if (!isset($order_id)){
        klog("setOrderPaid()", "failed, order_id is empty");
